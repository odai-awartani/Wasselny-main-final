...
export default function Profile() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (status !== 'granted') {
        setErrorMessage(language === 'ar' ? 'لم يتم منح إذن الوصول إلى الموقع' : 'Location permission not granted');
  ...
  const MapModal = () => {
      ...
      if (showMapModal && user?.location && currentLocation) {
        ...
        const fetchRoute = async () => {
          try {
            ...
            setErrorMessage(language === 'ar' ? 'تعذر جلب بيانات المسار' : 'Failed to fetch route data');
            setShowErrorModal(true);
          } finally {
            setIsLoading(false);
          }
        ...
        setIsLoading(false);
        setErrorMessage(language === 'ar' ? 'بيانات الموقع غير متوفرة' : 'Location data not available');
        setShowErrorModal(true);
      }
    ...
    const getInitialRegion = () => {
      ...
      const midLatitude = (currentLocation.latitude + user.location.latitude) / 2;
      const midLongitude = (currentLocation.longitude + user.location.longitude) / 2;
      // Calculate delta to encompass both points
      const latDelta = Math.abs(currentLocation.latitude - user.location.latitude) * 1.5;
      const lngDelta = Math.abs(currentLocation.longitude - user.location.longitude) * 1.5;
        ...
        <SafeAreaView className="flex-1 bg-white">
          ...
          {isLoading || !user?.location || !currentLocation ? (
            <View className="flex-1 justify-center items-center">
              ...
              <Text className={`mt-4 ${language === 'ar' ? 'font-CairoRegular' : 'font-JakartaRegular'} text-gray-600`}>
                ...
                <View className="px-4 pb-4">
                  <View className="flex-row items-center mb-4">
                    <View className="w-10 h-10 bg-orange-100 rounded-full items-center justify-center">
                      <Image source={icons.pin} className='w-6 h-6' tintColor={"white"} resizeMode='contain' />
                    </View>
                    <View className="flex-1 mx-3">
                      <Text className={`text-sm ${language === 'ar' ? 'font-CairoRegular text-right' : 'font-JakartaRegular text-left'} text-gray-500`}>
                        {language === 'ar' ? 'الموقع' : 'Location'}
                      </Text>
                      <Text className={`text-base ${language === 'ar' ? 'font-CairoBold text-right' : 'font-JakartaBold text-left'} text-gray-900`}>
                        {user.location.name}
                      </Text>
                    </View>
                  </View>
        ...
        >
          <View className="flex-1 justify-center items-center bg-black/50">
            <View className="bg-white p-6 rounded-xl w-4/5">
              <Text className={`text-lg ${language === 'ar' ? 'font-CairoBold text-right' : 'font-JakartaBold text-left'} text-gray-900 mb-4`}>
                {language === 'ar' ? 'خطأ' : 'Error'}
              </Text>
              <Text className={`text-sm ${language === 'ar' ? 'font-CairoRegular text-right' : 'font-JakartaRegular text-left'} text-gray-600 mb-6`}>
                {errorMessage}
              </Text>
              <TouchableOpacity
            ...
            {user.location && (
              ...
              >
                <View className="w-8 h-8 bg-green-100 rounded-full items-center justify-center">
                  <Image source={icons.pin} className='w-4 h-4' resizeMode='contain' tintColor={"black"}/>
                </View>
